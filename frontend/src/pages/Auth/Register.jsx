  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAlertModal({
        ...
        message: 'Akun pembeli Anda siap digunakan. Silakan masuk untuk mulai berbelanja di AmbaCart.',
        onConfirm: () => navigate('/login')
      });
    } catch (error) {
      console.error(error);
      const errorMsg = error.response?.data?.message || 'Gagal mendaftar. Periksa kembali data Anda.';
      
      setAlertModal({
        show: true, type: 'error', title: 'Registrasi Gagal', message: errorMsg,
  ...
  return (
    <div className="card shadow-lg border-0 rounded-4 p-4 p-md-5 bg-white position-relative" style={{ maxWidth: '400px', margin: '80px auto 20px auto', overflow: 'visible' }}>
      ...
      <div className="text-center mb-4 position-relative" style={{ marginTop: '-75px', zIndex: 10 }}>
        {/* FIXED: Maskot hanya berubah berdasarkan state ikon showPassword */}
        <img 
        ...
        />
        <h3 className="fw-black text-dark mb-1">Daftar Pembeli</h3>
        <p className="text-secondary small fw-medium mb-0">Gabung dan nikmati gratis ongkirnya!</p>
      </div>

      <form onSubmit={handleSubmit}>
        <div className="mb-3">
          <label className="form-label fw-bold small text-dark">Nama Lengkap</label>
          <input type="text" name="name" className="form-control p-3 bg-light amba-input-field" placeholder="Ketik nama lengkap..." value={formData.name} onChange={handleChange} required />
        </div>

        <div className="mb-3">
          <label className="form-label fw-bold small text-dark">Email</label>
          <input type="email" name="email" className="form-control p-3 bg-light amba-input-field" placeholder="nama@email.com" value={formData.email} onChange={handleChange} required />
        </div>
